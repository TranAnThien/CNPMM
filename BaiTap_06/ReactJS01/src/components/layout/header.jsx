...
const Header = () => {
    ...
    const navigation = useMemo(() => [
        ...
        { label: 'Đơn hàng', to: '/order/history' },
    ...
    const handleLogout = async () => {
        const ok = await showConfirm('Bạn có chắc chắn muốn đăng xuất?');
        if (!ok) return;
        ...
        setMenuOpen(false);
        showToast('Bạn đã đăng xuất', { type: 'success' });
        navigate('/');
    };
                ...
                <div className="flex items-center gap-3">
                    {auth?.isAuthenticated ? (
                        <>
                            <Link
                                to="/cart"
                                className="relative inline-flex h-11 w-11 items-center justify-center rounded-full border border-slate-300 bg-white text-slate-700 transition hover:bg-slate-100"
                                aria-label="Giỏ hàng"
                            >
